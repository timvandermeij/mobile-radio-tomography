...
void radioInit(rfConfig_t *rfConfig) {
    ...
    TXPOWER = (char)rfConfig->txPower; // Transmitter power
    FREQCTRL = 11 + 5 * ((char)(rfConfig->channel - 11)); // Frequency setup: f=(2394 + FREQCNTL) MHz
...
char receivePacket(char* ptr, char len, signed char* rssi) {
    ...
    if(RXFIFOCNT != len + 14) {
        RFST = ISFLUSHRX;
        RFST = ISFLUSHRX; //Flush the FIFO (issue twice)
        return 0; // Error
    }
