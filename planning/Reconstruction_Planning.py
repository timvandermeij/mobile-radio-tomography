...
def KLP(P, Objectives):
    """
    Perform the Kung, Luccio, and Preparata algorithm to determine nondominated
    solutions. The given list P must contain objective values for exactly two
    objective functions. It must be sorted in the first objective value already.

    We return a dictionary of objective values that should be kept,
    indexed by the original individual indices and a list of indices that should
    be removed from P for another run because they are nondominated.
    """

    # For now just 2D-KLP, we don't need anything else in our cases.
    T = OrderedDict()
    # Since we perform minimization, we want to find lower values instead of 
    # higher values like in the original 2D-KLP algorithm. So swap the yStar.
    yStar = np.inf
    todelete = []
...
def sort_nondominated(Objectives):
    """
    Sort a list of objective values for individuals into groups of nondominated
    and dominated solutions.

    The resulting list contains dictionaries with the original indices of the
    given list and their objective values. The first dictionary in this list
    are nondominated individuals, the rest are dominated.
    """

    # Sort P by first coordinate in ascending order, since we do minimization. 
    # P only contains the keys the the objective values of the points.
    ...
    # next group after one go? They are all nondominated anyway and probably do 
    # not use them for anything else.
    while len(P) > 0:
        Rk, todelete = KLP(P, Objectives)
        R.append(Rk)
        # Need to delete in reverse order so that the subsequent indexes are 
        # still correct.
...
def crowding_distance(Rk):
    """
    Calculate the crowding distances of individuals for the NSGA-II algorithm.
    The individuals are nondominated and have their objective values. This
    function can work with any number of objective functions.

    The resulting list contains values that indicate how useful each individual
    is to have in a potential Pareto front; points that are less close to others
    are more useful to keep.
    """
...
def hypervolume_contribution(Rk):
    """
    Calculate the 2D hypervolume contribution of individuals for the SMS-EMOA
    algorithm. The individuals are nondominated and have their objective values.

    The resulting list contains values that indicate how useful each individual
    is to have in a potential Pareto front; points that have a larger 
    hypervolume around themselves are more useful to keep.
    """
...
def evolve(problem, algo, mu, t_max, steps):
    """
    ...
    steps = np.array((steps * ((problem.dim / len(steps)) + 1))[:problem.dim])
    print("Settings: Problem {}, Algo {}, mu={}, t_max={}".format(p, algo, mu, t_max))
    print("Steps: {}".format(steps))

    # Generate mu random vectors in feasible domaim using domain specification.
    P = [problem.get_random_vector() for _ in range(mu)]
    #print("Initial population: {}".format(P))

    # Evaluate objectives and constraints for points in P.
    Feasible, Objectives = problem.evaluate(P)
    ...
    for t in xrange(1, t_max+1):
        ...
        if len(Delete) == 0:
            R = sort_nondominated(Objectives)
        ...
        if len(Delete) > 0:
            ...
            if algo == "NSGA":
                C = crowding_distance(R[0])
            else:
                C = hypervolume_contribution(R[0])
